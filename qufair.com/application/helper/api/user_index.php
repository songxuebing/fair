    ...
    switch($this->Param['option']){
		case 'submit'://修改企业资料
			...
			**/
			...
			$company = urldecode($this->Param['company']);
			if(!empty($company)){
				$data['company'] = urldecode($this->Param['company']);
			}
			...
			$addr = urldecode($this->Param['address']);
			if(!empty($addr)){
				$data['address'] = urldecode($this->Param['address']);
			}
			$ship_province = urldecode($this->Param['ship_province']);
			if(!empty($ship_province)){
				$menberData['province'] = urldecode($this->Param['ship_province']);
			}
			$ship_city = urldecode($this->Param['ship_city']);
			if(!empty($ship_city)){
				$menberData['city'] = urldecode($this->Param['ship_city']);
			}
			$ship_area = urldecode($this->Param['ship_area']);
			if(!empty($ship_area)){
				$menberData['area'] = urldecode($this->Param['ship_area']);	
			}
			$username = urldecode($this->Param['username']);
			if(!empty($username)){
				$row = $MemberListHelper->GetMemberRow(array('`username` =?'=>$username));
				if(!empty($row)){
					$row['code'] = 1;
					$row['msg'] = '用户已经存在';
					echo $this->Param['callback']."(".json_encode($row).")";
				...
				$menberData['username'] = urldecode($this->Param['username']);	
			}
			
			$signature = urldecode($this->Param['signature']);
			if(!empty($signature)){
				$menberData['signature'] = urldecode($this->Param['signature']);	
			}

			
			$where['`id` = ?'] = $id;
			$MemberListHelper->memberUpdate($menberData, $where);
			
			if($id > 0){
				$res = $MemberDetailHelper->DetailGetId($where);
				if($res > 0){
					if(!empty($data)){
						$MemberDetailHelper->DetailUpdate($data,$where);
					}
				}else{
					$data['id'] = $id;
		...
		case 'uploadImg'://上传企业头像
			...
			$data = $EditorUploadHelper->ImageUploadApp($this->Param['filebox'],'avatar',$member_id);
