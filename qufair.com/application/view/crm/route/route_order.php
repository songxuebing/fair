...
<body class="font_14px">
	...
	<div class="table_div_05"><span class="float_left"><a href="#">行程管理</a>&nbsp;&gt;&nbsp;<a href="#">行程订单</a></span><span class="float_right"></span></div>
	<div class="clear_15px"></div>
	<div class="table_div_04" style="padding-left:5px;">
	<form action="/route/order/" method="post" name="search" autocomplete="off">
	...
	<div class="clear_15px"></div>
	<div class="tab_title">
		<a href="/route/order/" class="<?php echo $this->param['list']=='' ? 'tab_01_ac' : '';?>">全部</a>
		<a href="/route/order/?list=waitpay" class="<?php echo $this->param['list']=='waitpay' ? 'tab_01_ac' : '';?>">待支付</a>
		<a href="/route/order/?list=waitreceive" class="<?php echo $this->param['list']=='waitreceive' ? 'tab_01_ac' : '';?>">待收货</a>
		<a href="/route/order/?list=finish" class="<?php echo $this->param['list']=='finish' ? 'tab_01_ac' : '';?>">已完成</a>
		<a href="/route/order/?list=cancel" class="<?php echo $this->param['list']=='cancel' ? 'tab_01_ac' : '';?>">已取消</a>
	</div>
	<div class="table_div_01">
	  <ul>
      	
		<li class="li_10">订单编号</li>
		<li class="li_15">购买行程</li>
	...
	<div class="table_div_02 J-table_div_02">
	  ...
	  <div class="list_info">
		<ul class="row">
          ...
          <li class="li_15"><a href="<?php echo WEB_URL.'/route/index/option/detail/id/'.$v['order_row']['route_id'];?>" target="_blank"><?php echo $v['order_row']['goods_name'];?></a></li>
          <li class="li_10"><?php echo $v['receiving']['company_name'];?></li>
		  <li class="li_5"><?php echo $v['receiving']['contacter'];?></li>
	...
	jQuery(document).ready(function(e) {
        jQuery(".J-quxiao").on('click',function(){
			if(confirm('您确定要取消订单吗？')){
				...
				jQuery.post('/route/order/option/cancel/',{'order_id':orderId},function(data){
					if(data.success){
		...
		jQuery(".J-daifu").on('click',function(){
			if(confirm('您确定要给此订单代付款吗？')){
				var orderId = jQuery(this).data('id');
				jQuery.post('/route/order/option/paid/',{'order_id':orderId},function(data){
					if(data.success){
						window.location.reload();
					}else{
						alert('代付失败');
