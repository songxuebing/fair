    ...
    public function shuntOrder($params) {
        switch ($params['is_type']) {
            case 'convention':
                ...
                $params['pay_return'] = '/convention/order/option/payreturn/sn/' . $params['order_sn'];
                break;
            case 'route':
            ...
            case 'visa':
				...
				$visa_row = $this->VisaModel->GetRow(array(
					'`visa_id` = ?' => $detail['visa_id']
				));
				$detail['visa_qq'] = $visa_row['visa_qq'];
                $detail['receiving'] = unserialize($detail['receiving']);
