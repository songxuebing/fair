...
<div class="mm_dingdan_detail mm_mid">
    ...
    <div class="ms_seach clearfix">
		...
		<form action="/route/order/" method="post" name="search">
    ...
    <div>
        ...
        <div class="mm_zh_dingdan_con">
            <ul class="clearfix">
                <li>
                    <dl>
                        <dt><img src="<?php echo Common::AttachUrl($v['goods_detail']['cover']);?>!a200" width="90" height="90"></dt>
                        <dd>
                            <h6><?php echo $v['goods_detail']['name'];?></h6>
                            <p>出发地点：<?php echo $v['goods_detail']['leave_area'];?></p>
                            <p>行程时间：<?php echo date('Y-m-d',$v['goods_detail']['leave_time']);?>至<?php echo date('Y-m-d',$v['goods_detail']['end_time']);?></p>
                        </dd>
                    </dl>
                </li>
                <li><div><?php echo $v['nums'];?></div></li>
                <li><em>￥<?php echo $v['order_row']['price'];?></em></li>
                <li><div><?php echo date('Y-m-d H:i:s',$v['dateline']);?></div></li>
            ...
            <div class="mm_zh_dingdancon">
                ...
                <div>
					...
					<a href="<?php echo MEMBER_URL;?>/public/common/option/contract/sn/<?php echo $v['order_sn'];?>" class="mm_zh_dingdan_xiugai" target="_blank">下载合同</a>
					<?php }?>
                    <a href="/route/order/option/detail/id/<?php echo $v['id'];?>" class="mm_zh_dingdan_detail" target="_blank">订单详情</a>
                </div>
            </div>
        </div>
		<?php
...
<script type="text/javascript">
	...
	jQuery("#mm_xiajia").on('click','.J-queding',function(){
		...
		jQuery.post('/route/order/option/changeprice/',{'id':id,'money':money},function(data){
			if(data.success){
