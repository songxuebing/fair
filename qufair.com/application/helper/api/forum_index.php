...
if(empty($this->Param['option'])){
	/**后期开始
	//官方政策列表
	$quzhan = $ForumHelper->catAll('13');
	$quzhanid = "'`cat_id` = ?' => 13";
	if(!empty($quzhan)){
		foreach($quzhan as $k => $v){
			$quzhanid .= " OR '`cat_id` = ?' => ".$v['id'];	
		}	
	}
	echo $quzhanid;
	die();
	$authority = $ForumHelper->forumPageList(array('`delete` = ?' => 0,'`is_show` = ?' => 1, $quzhanid), 5, 1 ,$this->Param);
	//知识列表
	$zhishi = $ForumHelper->catAll('40');
	$zhishiid = "'`cat_id` = ?' => 40";
	if(!empty($zhishi)){
		foreach($zhishi as $k => $v){
			$zhishiid .= " OR '`cat_id` = ?' => ".$v['id'];	
		}	
	}
	$knowledge = $ForumHelper->forumPageList(array('`delete` = ?' => 0,'`is_show` = ?' => 1, $zhishiid), 5, 1 ,$this->Param);
	//行业列表
	$hangye = $ForumHelper->catAll('148');
	$hangyeid = "'`cat_id` = ?' => 148";
	if(!empty($hangye)){
		foreach($hangye as $k => $v){
			$hangyeid .= " OR '`cat_id` = ?' => ".$v['id'];	
		}	
	}
	$industry = $ForumHelper->forumPageList(array('`delete` = ?' => 0,'`is_show` = ?' => 1, $hangyeid), 5, 1 ,$this->Param);
	...
	*****/
    ...
    switch ($this->Param['option']){
		...
		case 'focus':
			...
			* userId  用户id
			*/
			$userId = empty($this->Param['userId']) ? 0 : $this->Param['userId'];
			if(empty($this->Param['fenleiId'])){
				$row = array(
                    'code'=>1,
                    'msg'=>'请选择关注的分类'
                );
            ...
            $MemberDetailHelper->Update(array(
                'forum_id' => $this->Param['fenleiId']
            ),array(
                '`id` = ?' => $userId
            ));
