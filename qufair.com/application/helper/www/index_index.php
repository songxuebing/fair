...
//        }
        ...
        //调取最新资讯
        $new = $ForumHelper->forumPageList(array('`delete` = ?' => 0,'`is_show` = ?' => 1), 5, 1 ,$this->Param);
        //var_dump($new);exit();
        $this->Assign('new',$new);
        //调用知识百科的新闻
        ...
        //调用点击量高的新闻
        $new_hot = $ForumHelper->queryDetail('SELECT id,title,dateline,cover,content FROM `dyhl_forum`  JOIN dyhl_forum_tagindex WHERE dyhl_forum.id = dyhl_forum_tagindex.forum_id and dyhl_forum.is_show = 1 and dyhl_forum.delete = 0 and dyhl_forum.recommend = 0 ORDER BY rand() DESC LIMIT 8');
        //var_dump($new_ke);exit();
