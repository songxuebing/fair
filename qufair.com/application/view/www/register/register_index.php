...
<div class="mm_denglu" style="background:url(<?php echo STYLE_URL;?>/style/quzhan/images/20160202/mm_img01_02.jpg) no-repeat; background-position: 50% 50%;">
    <div class="mm_mid">
        <div class="mm_denglu_con1 fr" style="height:auto;">
            <div class="mm_form">
            	<div class="reg-box">
                	<a href="/register/index" class="on">参展商注册</a>
                    <a href="<?php echo MERCHANT_URL;?>/auth/register/" style="position:relative; top:10px;">展会服务商注册</a>
                </div>
                <form action="/register/index/" class="AjaxForm brand" method="post" autocomplete="off">
                <ul>
                    <li>
...
jQuery(document).ready(function(e) {
	//标签删除
	jQuery(".J-my-tag-box").on('click','a',function(){
		...
		$this.closest('dd').fadeOut('fast',function(){
			...
			var html="";
			jQuery(".J-my-tag-box").find('dd').each(function(index, element) {
                html+=jQuery(element).find("em").text()+' ';
            });
			jQuery("#mytag").val(html);
	...
	jQuery(".J-mm-yanzheng").on('click',function(){
		...
		if(flag){
			jQuery.post('/register/index/option/sendcode/',{'emialmobile':emialmobile,'username':'username','isemail':isemail,'code':code},function(data){
				if(data.success){
					flag = false;
...
function sucessCallback(){
	artDialog('注册成功，请登陆','succeed','window.location.href = "/login/index/";');
