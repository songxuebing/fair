...
<script type="text/javascript" src="<?php echo STYLE_URL;?>/style/quzhan/js/lightbox.js"></script>
...
<div class="mm_mid">
    ...
    <div class="mm_zh_detail clearfix">
            ...
            <div class="examples_body">
                <div class="examples_bg">
                    <div class="examples_image J-examples_image">
                        <img src="<?php echo Common::AttachUrl($this->data['cover']);?>!a200" alt="" style="background:url(<?php echo STYLE_URL;?>/style/quzhan/images/loading.gif) no-repeat 50% 50%;" />
                    </div>
                    <div class="mune_thumb J-mune_thumb">
                        <ul>
                            <li class="active">
                                <a data-href="<?php echo Common::AttachUrl($this->data['cover']);?>!a200" href="javascript:;"><img src="<?php echo Common::AttachUrl($this->data['cover']);?>!a200" alt="Image Name" style="background:url(<?php echo STYLE_URL;?>/style/quzhan/images/loading.gif) no-repeat 50% 50%;" /></a>
                            </li>
                            <?php
        ...
        <form action="/decoration/order" method="post">
        ...
        <div class="mm_zh_detail_right fr">
            ...
            <ul class="mm_tzfw">
                <li>
                    <span>面积大小：</span>
                    <em><?php echo $this->data['proportion'];?>平方</em>
                </li>
                <li class="J-style-img">
                    <span>装修风格：</span>
                    <?php 
						if(!empty($this->data['style_img'])) foreach($this->data['style_img'] as $key => $val){
							if(!empty($val)){
					?>
                    <label data-style="<?php echo $key;?>" style="margin-right:10px;"><a href="<?php echo Common::AttachUrl($val);?>" data-lightbox="roadtrip" title="风格 <?php echo $key+1;?>"><img src="<?php echo Common::AttachUrl($val);?>" width="52" height="52"></a></label>
                ...
                <li>
                    <span>展会规格：<?php echo $this->data['dec_area	'];?></span>
                    
                    <span>有效期：<em><?php echo $this->data['start_time'] == 0 ? '' : date('Y-m-d',$this->data['start_time']);?> - <?php echo $this->data['end_titme'] == 0 ? '' : date('Y-m-d',$this->data['end_titme']);?></em></span>
              </li>
              <li class="mm_xingcheng_jiage">￥<?php echo $this->data['de_price'];?><span>/套</span></li>
            </ul>
            
            <div class="clearfix">
                <div class="mm_xingcheng_bus">
                	<input type="hidden" name="style_img" value="0" />
                    ...
                    <!--<a href="/logistics/order/id/<?php echo $this->data['id'];?>">立即购买</a>-->
                </div>
                <div class="mm_xingcheng_xianxi">
                    <a href="tencent://message/?uin=<?php echo $this->data['qq'];?>&Site=http://www.qufair.com&Menu=yes" target="_blank">联系我</a>
                </div>
				<style type="text/css">
					.mm_guanzhu em {
...
jQuery(function(){
	jQuery('.J-mm-guanzhu').on('click',function(){
		...
		jQuery.post('/public/common/option/favorite/',{'sort':3,'related_id':<?php echo $this->data['id'];?>},function(data){ //state状态 
			if(data.success){
				if(flag == 1){
	...
	//选择风格
	jQuery(".J-style-img").on('click','label',function(){
		var $this = jQuery(this);
		
		var num = $this.data('style');
		jQuery('input[name="style_img"]').val(num);
		$this.addClass("border-label");
		$this.siblings('label').removeClass('border-label');
	});
...
<div class="mm_mid clearfix">
    ...
    <div class="mm_zhanhui_left fl">
    	<div style="margin-bottom:20px;"><img src="<?php echo STYLE_URL;?>/style/quzhan/images/decoration.png" /></div>
        <!-- 展位供应商 -->
        ...
        <div class="mm_zhanhui_con1 clearfix">
            ...
            <div class="mm_zhanhui_pingfen">
                ...
                <input type="hidden" name="is_type" value="6">
    ...
    <div class="mm_zhanhui_right fr">
		...
		<script type="text/javascript">
			jQuery.get('/public/index/option/data',{'output':'json','type':'route','key':'<?php echo $this->data['countries'];?>'},function(data){
				var bt=baidu.template;
				var html=bt('new-xingcheng',data);
				document.getElementById('J-new-xingcheng').innerHTML=html;
			
			},'json');
			jQuery.get('/public/index/option/data',{'output':'json','type':'visa','key':'<?php echo $this->data['countries'];?>'},function(data){
				var bt=baidu.template;
				var html=bt('new-qz',data);
				document.getElementById('J-new-qz').innerHTML=html;
			
			},'json');
			jQuery.get('/public/index/option/data',{'output':'json','type':'logistics','key':'<?php echo $this->data['countries'];?>'},function(data){
				var bt=baidu.template;
				var html=bt('new-wl',data);
				document.getElementById('J-new-wl').innerHTML=html;
			
			},'json');
			jQuery.get('/public/index/option/data',{'output':'json','type':'decoration','key':'<?php echo $this->data['countries'];?>'},function(data){
				var bt=baidu.template;
...
jQuery(function(){
	...
	jQuery(".J-log-freight").on('change',function(){
		...
		jQuery('input[name="J-freight-txt"]').val(txt);
	});
});
